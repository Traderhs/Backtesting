...
VolumeSimpleMovingAverage::VolumeSimpleMovingAverage(const string& name,
  ...
  if (!isfinite(period) || period <= 0 || floor(period) != period) {
    throw runtime_error(
        format("VolumeSimpleMovingAverage 지표의 Period [{}]은(는) 유한한 "
               "양의 정수여야 합니다.",
               period));
  }
...
void VolumeSimpleMovingAverage::Initialize() {
...
Numeric<double> VolumeSimpleMovingAverage::Calculate() {
  const double volume =
