...
void OrderHandler::MarketEntry(const string& entry_name,
  ...
  try {
    ...
    try {
      ...
      IsValidPositionSize(order_size, MARKET, symbol_idx);
    ...
    if (entry_now) {
      ...
      ExecuteMarketEntry(market_entry, symbol_idx, CLOSE);
    } else {
...
void OrderHandler::LimitEntry(const string& entry_name,
  ...
  try {
    ...
    try {
      ...
      IsValidPositionSize(order_size, LIMIT, symbol_idx);
      IsValidNotionalValue(order_price, order_size, symbol_idx);
      IsValidLeverage(leverage, order_price, order_size, symbol_idx);
    } catch (const InvalidValue& e) {
      LogFormattedInfo(WARNING_L, e.what(), __FILE__, __LINE__);
...
void OrderHandler::MitEntry(const string& entry_name,
  ...
  try {
    ...
    try {
      ...
      IsValidPositionSize(order_size, MIT, symbol_idx);
...
void OrderHandler::LitEntry(const string& entry_name,
  ...
  try {
    ...
    try {
      ...
      IsValidPositionSize(order_size, LIT, symbol_idx);
      IsValidNotionalValue(order_price, order_size, symbol_idx);
      IsValidLeverage(leverage, order_price, order_size, symbol_idx);
    } catch (const InvalidValue& e) {
      LogFormattedInfo(WARNING_L, e.what(), __FILE__, __LINE__);
...
void OrderHandler::TrailingEntry(const string& entry_name,
  ...
  try {
    ...
    try {
      ...
      IsValidPositionSize(order_size, TRAILING, symbol_idx);
...
void OrderHandler::MarketExit(const string& exit_name,
  ...
  try {
    ...
    if (const auto& result = FindEntryOrder(target_name, symbol_idx)) {
      const auto& [order, index] = result.value();
      entry_order = order;
...
void OrderHandler::LimitExit(const string& exit_name, const string& target_name,
  ...
  try {
    ...
    shared_ptr<Order> entry_order;
    if (const auto& result = FindEntryOrder(target_name, symbol_idx)) {
      entry_order = result.value().first;
    } else {
      // 원본 진입 주문을 찾지 못하면 청산 실패
      LogFormattedInfo(
          WARNING_L,
    ...
    switch (pending_exit->GetExitOrderType()) {
      ...
      case LIMIT: {
        if (const auto& order_info =
          ...
          should_fill_pending_exits.push_back(order_info.value());
        }
    ...
    switch (const auto& pending_entry = pending_entries[order_idx];
      ...
      case LIMIT: {
        if (const auto& order_info =
          ...
          should_fill_pending_entries.push_back(order_info.value());
        }
...
void OrderHandler::FillOrder(const FillInfo& order_info, const int symbol_idx,
  ...
  try {
    switch (const auto& [order, order_signal, fill_price] = order_info;
            order_signal) {
      case LIQUIDATION: {
        // 먼저 다른 주문에서 원본 진입 주문을 청산했으면 강제 청산의 의미가
        // 없으므로 바로 리턴
      ...
      case EXIT: {
        ...
        if (FindEntryOrder(order->GetEntryName(), symbol_idx)) {
          FillPendingExitOrder(order, symbol_idx, fill_price);
        }
        ...
        switch (order->GetEntryOrderType()) {
          ...
          default: {
            Logger::LogAndThrowError(
                "잘못된 전략 타입 ORDER_NONE이 지정되었으므로 주문을 실행할 수 "
                "없습니다.",
                __FILE__, __LINE__);
          }
        }
...
void OrderHandler::ExecuteMarketEntry(const shared_ptr<Order>& market_entry,
  ...
  // 내부 처리가 편리하기 때문
  try {
    AdjustLeverage(market_entry->GetLeverage(), symbol_idx);
  } catch (const InvalidValue& e) {
    LogFormattedInfo(WARNING_L, e.what(), __FILE__, __LINE__);
    throw OrderFailed("시장가 진입 실패");
  }
...
void OrderHandler::ExecuteExit(const shared_ptr<Order>& exit_order,
  ...
  if (const auto& result = FindEntryOrder(entry_name, symbol_idx)) {
    const auto& entry_order = result.value().first;
...
optional<FillInfo> OrderHandler::CheckPendingLitEntry(
  ...
  if (lit_entry->GetEntryOrderTime() == -1) {
    if (IsPriceTouched(lit_entry->GetEntryTouchDirection(), price,
      ...
      try {
        // 터치 시 지정가 주문 접수
        OrderPendingLitEntry(lit_entry, order_idx, symbol_idx, price_type);
      } catch ([[maybe_unused]] const OrderFailed& e) {
...
void OrderHandler::FillPendingMarketEntry(const shared_ptr<Order>& market_entry,
  ...
  // 예외 체크
  try {
    IsValidEntryName(market_entry->GetEntryName(), symbol_idx);
  } catch (const InvalidValue& e) {
    LogFormattedInfo(WARNING_L, e.what(), __FILE__, __LINE__);
  ...
  // 자금 관련 처리 후 체결 주문에 추가
  ExecuteMarketEntry(market_entry, symbol_idx, price_type);
}
...
void OrderHandler::FillPendingLimitEntry(const shared_ptr<Order>& limit_entry,
  ...
  // 레버리지 설정
  // 사용자가 직접 AdjustLeverage를 하지 못 하게 한 이유는,
  // 반대 방향 주문을 확실하게 청산하고 목표하는 레버리지로 확실하게 진입할 수
  // 있게 하기 위함과, 목표 레버리지를 진입 시점에 직접 변경하는 것보다 엔진
  // 내부 처리가 편리하기 때문
  try {
...
void OrderHandler::OrderPendingLitEntry(const shared_ptr<Order>& lit_entry,
...
void OrderHandler::FillPendingExitOrder(const shared_ptr<Order>& exit_order,
                                        const int symbol_idx,
                                        const double fill_price) {
  // 현재 바 시간 로딩
  const auto current_open_time = engine_->GetCurrentOpenTime();
  ...
  if (const auto& result = FindEntryOrder(target_name, symbol_idx)) {
    ...
    LogFormattedInfo(
        ...
        __FILE__, __LINE__);
    throw OrderFailed("엔진 오류: 대기 주문 청산 시 원본 진입 주문 미존재");
  }
