...
void DiceSystem::ExecuteAfterEntry() {
  ...
  if (current_position_size_ > 0) {
    ...
    order->MitExit("매수 손절", "매수 진입",
                   swing_high - abs(swing_high - swing_low) * long_stop_ratio_,
                   left_size);
    return;
  }
  ...
  if (current_position_size_ < 0) {
    ...
    order->MitExit("매도 손절", "매도 진입",
                   swing_low + abs(swing_high - swing_low) * short_stop_ratio_,
                   left_size);
  }
}
...
pair<double, int> DiceSystem::CalculatePositionSizeAndLeverage(
  ...
  if (isnan(stop_loss_points)) {
    OrderHandler::LogFormattedInfo(
        WARNING_L,
        "스톱 로스 포인트가 NaN이므로 포지션 크기와 레버리지 계산 불가",
        __FILE__, __LINE__);
  ...
  if (isnan(order_price)) {
    OrderHandler::LogFormattedInfo(
        WARNING_L, "진입 가격이 NaN이므로 포지션 크기와 레버리지 계산 불가",
        __FILE__, __LINE__);
    return make_pair(0, 0);
  }
  ...
  if (stop_loss_points == 0) {
    OrderHandler::LogFormattedInfo(
        WARNING_L,
        "스톱 로스 포인트가 0이므로 포지션 크기와 레버리지 계산 불가", __FILE__,
        __LINE__);
    return make_pair(0, 0);
  }
  ...
  if (order_price == 0) {
    OrderHandler::LogFormattedInfo(
        WARNING_L, "진입 가격이 0이므로 포지션 크기와 레버리지 계산 불가",
        __FILE__, __LINE__);
    return make_pair(0, 0);
  }
  ...
  if (const double min_notional_value = symbol_info.GetMinNotionalValue();
    ...
    OrderHandler::LogFormattedInfo(
        WARNING_L,
        format("계산된 명목 가치 [{}]가 해당 심볼의 최소 명목 가치 [{}]보다 "
               "적으므로 진입 불가",
