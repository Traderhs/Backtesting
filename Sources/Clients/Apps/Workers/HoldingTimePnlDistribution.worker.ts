// Sources/Clients/Apps/Workers/HoldingTimePnlDistribution.worker.ts
...
const parseHoldingTimeToSeconds = (holdingTimeStr: string): number => {
    if (!holdingTimeStr || holdingTimeStr === '-') {
        return 0;
    }

    // "동일봉 거래"인 경우 0초로 처리
    if (holdingTimeStr === '동일봉 거래') {
        return 0;
    }
...
const getMinimalUnitForZero = (nonZeroSeconds: number[]): string => {
    ...
    if (minSeconds < 60) {
        ...
        return '0시간';
    } else if (minSeconds < 604800) {
        return '0일';
    } else if (minSeconds < 2592000) {
        return '0주';
