...
const AdvancedEntryTimeFilterModal: React.FC<AdvancedEntryTimeFilterModalProps> = ({
    ...
    }, [isOpen, filter.entryYears, filter.entryMonths, filter.entryDays, filter.entryDayOfWeeks, filter.entryHours, filter.entryMinutes, filter.entrySeconds, setValues]);
    ...
    const handleApply = useCallback(() => {
        setFilter(prev => ({
            ...prev,
            entryYears: latestValues.current.entryYears || [],
            entryMonths: latestValues.current.entryMonths || [],
            entryDays: latestValues.current.entryDays || [],
            entryDayOfWeeks: latestValues.current.entryDayOfWeeks || [],
            entryHours: latestValues.current.entryHours || [],
            entryMinutes: latestValues.current.entryMinutes || [],
            entrySeconds: latestValues.current.entrySeconds || [],
        }));
        onClose();
    }, [setFilter, onClose]);
    ...
    const handleCancel = useCallback(() => {
        setValues({
            entryYears: latestFilter.current.entryYears,
            entryMonths: latestFilter.current.entryMonths,
            entryDays: latestFilter.current.entryDays,
            entryDayOfWeeks: latestFilter.current.entryDayOfWeeks,
            entryHours: latestFilter.current.entryHours,
            entryMinutes: latestFilter.current.entryMinutes,
            entrySeconds: latestFilter.current.entrySeconds,
        });
        onClose();
    }, [setValues, onClose]);
    ...
    const renderDayOfWeekSection = () => {
        ...
        const currentValues = values.entryDayOfWeeks || [];
        const isAllSelected = currentValues.length === options.length;
        const isIndeterminate = currentValues.length > 0 && currentValues.length < options.length;
        ...
        <div className="filter-calendar-overlay">
            ...
            >
                ...
                <div className="advanced-filter-content">
                    ...
                    {/* 월 */}
                    {renderTimeSection("월", monthOptions, "entryMonths")}

                    {/* 일 */}
                    {renderTimeSection("일", dayOptions, "entryDays")}

                    {/* 요일 - 커스텀 렌더링 함수 사용 */}
                    {renderDayOfWeekSection()}

                    {/* 시 */}
                    {renderTimeSection("시간", hourOptions, "entryHours")}
