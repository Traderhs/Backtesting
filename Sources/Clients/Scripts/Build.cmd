...
for /f "delims=" %%A in ('powershell -NoProfile -Command "(Get-ChildItem Apps -Recurse -Include *.css,*.js,*.ts,*.jsx,*.tsx -File -Exclude output.css | Sort-Object LastWriteTime -Descending | Select-Object -First 1).LastWriteTimeUtc.Ticks"') do set TW_SRC_TICKS=%%A
for /f "delims=" %%B in ('powershell -NoProfile -Command "(Get-Item Apps\output.css).LastWriteTimeUtc.Ticks"') do set OUT_TICKS=%%B
...
if "%NEEDS_TW_BUILD%"=="true" (
  ...
  call node_modules\\.bin\\tailwindcss-cli.cmd -c Configs\tailwind.config.js -i Apps\index.css -o Apps\output.css
  if errorlevel 1 (
...
if not exist BackBoard (
  ...
  set SRCB_TICKS=0
  for /f "delims=" %%C in ('powershell -NoProfile -Command "(Get-ChildItem Apps -Recurse -File | Sort-Object LastWriteTime -Descending | Select-Object -First 1).LastWriteTimeUtc.Ticks"') do set SRCB_TICKS=%%C
  set BACKB_TICKS=0
