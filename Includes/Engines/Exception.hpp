...
class BACKTESTING_API InvalidValue final : public runtime_error {
 public:
  explicit InvalidValue(const string& message) : runtime_error(message) {}
};

/// 인덱스가 범위를 벗어났을 때 발생하는 에러
class BACKTESTING_API IndexOutOfRange final : public runtime_error {
 public:
  explicit IndexOutOfRange(const string& message) : runtime_error(message) {}
};

/// 진입 가능 자금이 부족할 때 발생하는 에러
class BACKTESTING_API InsufficientBalance final : public runtime_error {
 public:
...
/// 파산 시 발생하는 에러
class BACKTESTING_API Bankruptcy final : public runtime_error {
 public:
