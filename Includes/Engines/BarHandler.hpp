...
class BarHandler final : public BaseBarHandler {
 public:
  ...
  /// 현재 사용 중인 심볼의 인덱스
  int current_symbol_idx_;

  /// BarHandler의 싱글톤 인스턴스를 반환하는 함수
  static BarHandler& GetBarHandler();
   ...
   */
  void AddBarData(const string& name, const string& file_path,
                  const vector<int>& columns, BarType bar_type);

  /// 바 데이터 타입 및 심볼과 타임프레임에 해당되는 바 데이터의
  /// 현재 인덱스를 설정하는 함수
  void SetCurrentBarIndex(size_t index, int symbol_idx,
                          const string& timeframe);

  /// 심볼과 타임프레임 및 바 데이터 타입에 해당되는 바 데이터의
  /// 현재 인덱스를 반환하는 함수
  size_t GetCurrentBarIndex(int symbol_idx, const string& timeframe = "");

  /// 현재 사용 중인 바의 타입을 반환하는 함수
  [[nodiscard]] BarType GetCurrentBarType() const;

  /// 현재 사용 중인 심볼의 인덱스를 반환하는 함수
  [[nodiscard]] int GetCurrentSymbolIdx() const;
 ...
 private:
  ...
  static mutex mutex_;
  static unique_ptr<BarHandler> instance_;
